...
void FOliveMCPPromptTemplates::RegisterDefaultTemplates()
{
	...
	// ==========================================
	{
		...
		Template.TemplateText =
			...
			TEXT("2. If creating something new, confirm your understanding of any unfamiliar node types ")
			TEXT("(blueprint.describe_node_type, blueprint.describe_function) and check olive.get_recipe for tested patterns.\n\n")
			...
			TEXT("4. Build Blueprint graphs one primitive at a time (blueprint.add_node, connect_pins, set_pin_default). ")
			TEXT("Compile once the full graph for a function is wired.\n\n")
	...
	// ==========================================
	{
		...
		Template.TemplateText =
			...
			TEXT("Check olive.get_recipe for tested wiring patterns matching your task domain.\n\n")
			TEXT("STRUCTURE + BUILD: Call tools directly — blueprint.create, add_component, add_variable, ")
			TEXT("add_function (for signatures), then add_node / connect_pins / set_pin_default for each graph edit, then compile.\n\n")
			TEXT("Granular tools are the default; Python (editor.run_python) handles anything the other tools cannot express.\n\n")
			...
			TEXT("Wire FROM them, not TO them with connect_pins. ")
			TEXT("Use blueprint.read to inspect pin names if unsure.");
	...
	// ==========================================
	{
		...
		Template.TemplateText =
			...
			TEXT("PLAN: Decide what needs to change. ")
			...
			TEXT("For graph logic changes, call blueprint.add_node for each node, connect_pins to wire, ")
			TEXT("set_pin_default for literals, and remove_node for anything being replaced. ")
			TEXT("Reuse existing event nodes (BeginPlay, Tick) by referencing their node IDs from blueprint.read ")
			TEXT("rather than adding duplicates.\n\n")
	...
	// ==========================================
	{
		...
		Template.TemplateText =
			...
			TEXT("FIX: Apply the smallest change that resolves the error — usually connect_pins, ")
			TEXT("set_pin_default, or remove_node + add_node to replace a single bad node.\n\n")
	...
	// ==========================================
	{
		...
		Template.TemplateText =
			...
			TEXT("EXISTING ASSETS: Use project.search and project.bulk_read to pull in similar assets already in the project ")
			TEXT("as reference. blueprint.read with section:'graph' mode:'full' shows node-level wiring.\n\n")
