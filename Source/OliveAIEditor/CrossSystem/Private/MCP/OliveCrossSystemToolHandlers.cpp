...
// =============================================================================
// Batch Write — Template Resolution Helper
// =============================================================================
 ...
 * @param OpResults     Map of opId -> result data JSON from prior ops
...
// =============================================================================
// Batch Write — Dispatch a single op to the appropriate writer method
// =============================================================================
	...
	const TSharedPtr<FJsonObject>& OpParams)
{
	...
	{
		FString NodeType = OliveToolParamHelpers::GetOptionalString(OpParams, TEXT("node_type"));
		if (NodeType.IsEmpty())
		{
			return FOliveBlueprintWriteResult::Error(TEXT("Missing required param 'node_type' for blueprint.add_node"));
		}
	...
	{
		FString Source = OliveToolParamHelpers::GetOptionalString(OpParams, TEXT("source_pin"));
		FString Target = OliveToolParamHelpers::GetOptionalString(OpParams, TEXT("target_pin"));
		if (Source.IsEmpty() || Target.IsEmpty())
		{
			return FOliveBlueprintWriteResult::Error(TEXT("Missing required params 'source_pin' and/or 'target_pin' for blueprint.connect_pins"));
		}
	...
	else if (ToolName == TEXT("blueprint.disconnect_pins"))
	{
		...
		if (Source.IsEmpty() || Target.IsEmpty())
		{
			return FOliveBlueprintWriteResult::Error(TEXT("Missing required params 'source_pin' and/or 'target_pin' for blueprint.disconnect_pins"));
		}
	...
	else if (ToolName == TEXT("blueprint.set_pin_default"))
	{
		...
		FString DefaultValue = OliveToolParamHelpers::GetOptionalString(OpParams, TEXT("default_value"));
		if (Pin.IsEmpty())
		{
			return FOliveBlueprintWriteResult::Error(TEXT("Missing required param 'pin' for blueprint.set_pin_default"));
		}
		return Writer.SetPinDefault(BlueprintPath, GraphName, Pin, DefaultValue);
	}
	else if (ToolName == TEXT("blueprint.set_node_property"))
	{
		...
		FString PropertyName = OliveToolParamHelpers::GetOptionalString(OpParams, TEXT("property_name"));
		FString PropertyValue = OliveToolParamHelpers::GetOptionalString(OpParams, TEXT("property_value"));
		if (NodeId.IsEmpty() || PropertyName.IsEmpty())
		{
			return FOliveBlueprintWriteResult::Error(TEXT("Missing required params 'node_id' and/or 'property_name' for blueprint.set_node_property"));
		}
