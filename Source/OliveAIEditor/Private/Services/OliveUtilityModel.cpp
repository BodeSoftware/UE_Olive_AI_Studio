...
{
	// Simple tokenizer: split on whitespace and common separators, lowercase, keep tokens >= 2 chars.
	TArray<FString> Tokens;
	{
		FString Working = UserMessage.ToLower();
		// Normalize separators to spaces
		Working = Working.Replace(TEXT("_"), TEXT(" "));
		Working = Working.Replace(TEXT("-"), TEXT(" "));
		Working = Working.Replace(TEXT("."), TEXT(" "));
		Working = Working.Replace(TEXT(","), TEXT(" "));
		Working = Working.Replace(TEXT("/"), TEXT(" "));
		Working.ParseIntoArrayWS(Tokens);
		Tokens.RemoveAll([](const FString& T) { return T.Len() < 2; });
	}
