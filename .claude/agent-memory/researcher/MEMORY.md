...
## Codex CLI & Claude Code Autonomous Mode (2026-03)
- Full report: `plans/research/codex-cli-and-claude-code-autonomous-mode-2026-03.md`
- Codex MCP config: `~/.codex/config.toml` under `[mcp_servers.<id>]`; both stdio (command/args) and HTTP (url) supported
- Codex context: `AGENTS.md` hierarchy (global→repo→subdirectory, root-to-leaf merge, 32 KiB cap); configurable fallback filenames
- Codex non-interactive: `codex exec --json` (JSONL events); `codex exec resume <ID>` for pipeline continuations
- Codex also EXPOSES ITSELF as MCP server via `codex mcp-server` (two tools: codex() + codex-reply())
- Claude Code session flags: `--session-id <uuid>` (deterministic), `--resume/-r <id-or-name>`, `--continue/-c` (most recent in dir), `--fork-session`
- Claude Code MCP: `.mcp.json` (project), `~/.claude/settings.json` (user); `--strict-mcp-config` for isolation
- `--dangerously-skip-permissions` skips all tool permission gates; prefer `--allowedTools "mcp__olive-ai-studio__*"` for production
- stream-json events: NDJSON lines; `type=="stream_event"` + `event.delta.type=="text_delta"` for text tokens; `system/api_retry` for retries
- `--permission-prompt-tool <mcp-tool>` routes permission prompts to an MCP tool (enables UE editor confirmation dialogs)

## Node Creation / Pin Interactivity
- `non-interactive-pins.md` — Root cause of non-draggable pins on `blueprint.add_node` nodes: missing `PostPlacedNewNode()`, `ReconstructNode()` orphaning old pins, Slate SGraphPin→UEdGraphPin stale pointers. Fix: call `PostPlacedNewNode()`, remove `ReconstructNode()` from base `CreateNodeByClass`, remove redundant `NotifyGraphChanged()` inside `CreateNodeByClass`.
- Research report: `plans/research/non-interactive-pins-ue55.md`

## Key Research Reports
- `plans/research/mcp-session-log-analysis-2026-03-20.md` — Pistol BP session (20 March). 12:01 agent time, 0 auto-continues. 1 BP built (BP_Pistol), compile SUCCESS. 42 tool calls, 88.1% success. plan_json 50% (3/6). 11 compile cycles ALL SUCCESS. NEW: olive.search_community_blueprints used first time (2:43 gap reading results). K2_SetTimer Object pin cascade ~4 min (silent already-wired error + wrong node ID from 39-node graph). 87 tools (think NOT present). CONFIRMED: call_delegate resolves all 3 dispatchers. Fire plan (19 steps) = most complex single-function plan seen. schema_version miss recurs (attempt 1). 10 recommendations filed.
- `plans/research/gun-bullet-session-log-analysis-2026-03-15b.md` — Gun/bullet/shooting run B (15 March). 4:12 agent time, 0 auto-continues, outcome=0. 3 BPs built (BP_Gun, BP_Bullet, BP_ThirdPersonCharacter), ALL compile SUCCESS. 42 tool calls, ~88% success. plan_json: 7/12 (58%). CONFIRMED FIXES: LATENT_IN_FUNCTION validator caught Delay in function graph (self-corrected to K2_SetTimer), CleanupPreviousPlanNodes cleaned 14 total nodes across 2 events, COLLISION_ON_TRIGGER_COMPONENT fires as warning (agent absorbed and added GunMesh collision disable). CONFIRMED WORKING: BulletMesh assigned Sphere/0.1 scale, MuzzlePoint world location used for spawn transform. PERSISTENT GAPS: (1) No GetControlRotation — bullets fire in gun forward direction not camera aim. (2) No socket name on AttachToComponent — gun attaches to mesh root not hand_r bone. (3) Legacy InputKey used instead of Enhanced Input IA asset. (4) Short type name "BP_ThirdPersonCharacter_C" in CheckVariableExists causes log noise (OlivePlanValidator.cpp:516). (5) OnActorBeginOverlap error message misleads agent to InputAction search. 10 recommendations filed.
- `plans/research/bullet-spawn-session-log-analysis-2026-03-15.md` — Bullet/projectile spawn run (15 March). 3:08 agent time, 27 tool calls, 0 auto-continues, exit 0. 3 BPs built (BP_Gun, BP_Bullet, BP_ThirdPersonCharacter), ALL compile SUCCESS. CRITICAL ISSUES: (1) MuzzlePoint get_var references non-existent variable on BP_Gun — bullet spawns at world origin 0,0,0; VARIABLE_NOT_FOUND Phase 0 check did NOT fire for this case (gap). (2) No aiming direction — no GetControlRotation/GetActorForwardVector; ProjectileMovement fires in world +X. (3) CollisionProfileName fails on StaticMeshComponent and SphereComponent via modify_component (0/1, 1/2). (4) EquippedGun typed Actor* instead of BP_Gun* — required explicit cast step (AI self-corrected on attempt 2). (5) OnShoot custom_event created but orphaned — InputKey wired directly to IsValid, bypassing it. (6) Legacy InputKey used instead of Enhanced Input. (7) First ThirdPersonCharacter plan rolled back — data wire failed Actor*→BP_Gun_C without cast. Tool success: ~85% (2 modify_component failures, 1 plan rollback). PIE sessions lasted <2 seconds each (user noticed bullets not working).
- `plans/research/gun-pickup-inventory-session-log-analysis-2026-03-13b.md` — Gun+pickup+inventory run B (13 March). 2 runs: Run 1 (50 calls, 9:32, gun+bullet+pickup, all BPs SUCCESS), Run 2 (~100 calls, ~29 min, BP_Item+inventory UI, 1 auto-continue, all 4 BPs SUCCESS). CONFIRMED FIXES: undraggable pins, C++ CDO component reading, component-to-component attachment, discovery-first. NEW ISSUES: (1) Array_Add wildcard ReconstructNode not called after connect_pins — 4 failures, ~12 min wasted; fix: check UK2Node_CallArrayFunction after wiring and call ReconstructNode. (2) set_parent_class Actor→Actor DefaultSceneRoot conflict — ~9 min wasted; fix: pre-flight guard. (3) modify_component fails before compile after add_component — timing issue; fix: better error message. (4) SetVisibility alias collision SceneComponent vs Widget — fix: remove from alias map or make fallback pin-aware. (5) VARIABLE_NOT_FOUND cross-BP get_var (3×, ~12 min) — fix: Phase 0 error message + knowledge note. (6) add_node CallFunction fails for uncompiled self-BP functions — K2Node_CallFunction workaround works. (7) VariableGet with foreign variable_class creates 0-pin ghost. (8) FMargin requires tuple format "(L,T,R,B)" — not documented. (9) Dotted sub-property path (Font.Size) not supported. 10 recommendations filed.
- `plans/research/gun-pickup-session-log-analysis-2026-03-13.md` — Gun+pickup run (13 March). 1 auto-continue (240s idle timeout). 11:23 total. 3 BPs compiled SUCCESS. 28 tool calls. CRITICAL ISSUES: (1) Wrong attachment — actor.AttachToComponent instead of GunMesh.AttachToComponent; no socket name set (hand_r). (2) 3 dispatchers (OnFired/OnAmmoEmpty/OnReloaded) created but never bound from BP_ThirdPersonCharacter — dead signals. (3) Nodes created via K2Node_CallFunction (local function calls) reported undraggable — CreateNodeByClass path with 3 pins, function_name reflection failure, SetFunctionReference fallback. (4) Pickup built from scratch with SphereComponent overlap, ignored existing project interactable interface — zero searches for "BPI", "IInteractable", or "interact". (5) blueprint.read returns 0 components for ACharacter descendants (C++ inherited components not in SCS). (6) blueprint.add_node type CallFunction fails 4× for local BP functions; K2Node_CallFunction workaround succeeds but produces fragile nodes. 10 recommendations filed.
...
- `plans/research/inventory-session-log-analysis-2026-03-12.md` — Inventory system run (12 March). SINGLE RUN, 0 auto-continues, 26:46 total. 7 BPs created, ALL compile SUCCESS. 50% plan_json (9/18). ~86% tool success (~128 total). Depth-first CONFIRMED. CastTargetMap inference CONFIRMED (AddToViewport via WBP_InventoryGrid_C cast). @entry param synthesis CONFIRMED (6× plans). Widget @ref synthesis CONFIRMED. Progressive timeout NOT triggered (<120s idle gaps throughout). Top failures: ReceiveConstruct wrong Widget event name (→Construct), Array_Add wildcard type undetermined (2×), FunctionEntry node ID unknown to connect_pins (4 failures, ~7min wasted), CallFunction/K2Node_CallFunction type normalization. 10 recommendations filed.
- `plans/research/inventory-session-log-analysis-2026-03-12b.md` — Inventory system run B (12 March, post-fix). SINGLE RUN, 0 auto-continues, 24:02 total. 7 BPs created, ALL compile SUCCESS. ~128 tool calls, ~86% success. Research phase WORKING (get_recipe×3, list_templates×1 before first create). bAutoCompile WORKING (every add_function triggers immediate compile). Widget OwnerCharacter wiring WORKING (cast→store→bind_dispatcher pattern correct). DOMINANT FAILURE: Array_Add wildcard type undetermined (3×, ~12 min wasted) — executor wires correctly but skips ReconstructNode call needed for type propagation. OTHER FAILURES: modify_only silent failure on inherited vars (9×, ~3 min), graph name collision (1×), AddToViewport no target_class (1×, self-corrected), StaticMesh direct assignment ensure (cosmetic bug in OliveComponentWriter.cpp:352). Top fixes needed: Array_Add ReconstructNode, OliveComponentWriter SetStaticMesh(), modify_only error message, UserWidget alias map, FMargin format docs. 10 recommendations filed.
- `plans/research/bow-arrow-session-log-analysis-2026-03-09t.md` — Run 09t: 7:38 agent time. 77.8% plan_json (7/9). 86.4% tool success (51/59). 59 total tool calls. All 3 BPs compiled SUCCESS. EXEC_SOURCE_IS_RETURN CONFIRMED (1×). CleanupStaleEventChains CONFIRMED firing. Exec auto-break CONFIRMED (1×). CustomEvent has no exec INPUT (knowledge gap). @Mesh ExpandComponentRefs gap. 10 recommendations filed.
- `plans/research/bow-arrow-session-log-analysis-2026-03-09s.md` — Run 09s: REGRESSION. 5:50 agent time. 55.6% plan_json (5/9). 88.4% tool success (38/43). All 3 BPs compiled SUCCESS. Fix 1 (FUNCTION_NOT_FOUND restructure) PARTIAL — agent self-corrected on missing target_class for Fire in 9s, but search trail was sufficient signal (old format also had it). Fix 2 (describe_function reframe) NOT WORKING — zero describe_function calls, agent ignores guidance entirely. Fix 3 (@get_class.auto) NOT EXERCISED — task used static asset paths. EXEC WIRE REJECTED ZERO (confirmed clean). PreResolvedFunction 27 hits (highest yet). New failure: FunctionOutput-as-exec-source AGAIN (1×, same as 09m/09q). IA_Fire still missing — agent pivoted to custom_event + InputKey correctly. Orphaned Branch node from prior failed attempt left in graph (compile tolerant). Dominant failures: FunctionOutput exec (1×), missing target_class (1×), IA_Fire missing (1×), data wire/architectural redesign (1×). 10 recommendations filed.
- `plans/research/bow-arrow-session-log-analysis-2026-03-09r.md` — Run 09r: 4:29 agent time. 66.7% plan_json (6/9). 91.9% tool success (34/37). All 3 BPs compiled SUCCESS. FUNCTION_NOT_FOUND directive guidance PARTIAL — agent never called describe_function (attempt 2 error was @get_class.auto crash, not FUNCTION_NOT_FOUND). Agent did adopt "fundamentally different approach" on attempt 3 (SetInstigator → set_var), which is the correct behavior. EXEC WIRE REJECTED GONE (confirmed). @get_class.auto executor bug RECURRED (from 09n) — Resolver accepts step-ref but Executor can't resolve it as class. SetVelocityInLocalSpace hallucination recurred (3rd+ run). SetInstigator hallucination self-corrected by agent on attempt 3. Fire function missing target_class fixed in 1 retry with target_class="BP_Bow". GetMesh→get_var("Mesh") fallback correct for Character BP. 10 recommendations filed.
- `plans/research/bow-arrow-session-log-analysis-2026-03-09q.md` — Run 09q: NEW BEST RUN. 4:09 agent time. 75% plan_json (6/8). 90% tool success (27/30). 4 BPs built, ALL compile SUCCESS. Fix 1 (Modify on reused nodes) CONFIRMED — EXEC WIRE REJECTED GONE. Fix 3 (Target/@step.auto) CONFIRMED — used 5× correctly. Fix 2 (exec auto-break) not exercised (not needed). Fix 4 (knowledge pack) CONFIRMED — agent used target_class + Target convention proactively. Dominant failures: SetVelocityInLocalSpace hallucination (2nd run with this bug), FunctionOutput-as-exec-source (1×, repeat from 09m), SetInstigator hallucination (not a UFunction). project.snapshot FAILS on pre-existing BPs (needs IR read). AttachToComponent fully wired for first time. 10 recommendations filed.
- `plans/research/bow-arrow-session-log-analysis-2026-03-09p.md` — Run 09p: NEW SPEED RECORD 7:02. 55.6% plan_json (5/9). 92.1% tool success (70/76). All BPs compile SUCCESS. EXEC WIRE REJECTED fired 1×, confirmed: root cause is exec source pin already having a link (links=1) from prior plan's partial commit on a reused node — NOT bOrphanedPin. Error message sent to agent is wrong ("TypesIncompatible") — needs fix. Reused nodes must be Modify()'d in transaction for rollback to sever their wiring. GetForwardVector alias fallback CONFIRMED WORKING. 10 recommendations filed.
- `plans/research/bow-arrow-session-log-analysis-2026-03-09o.md` — Run 09o: EnsurePinNotOrphaned NOT FIRING (bOrphanedPin bug fires 1×). PreResolvedFunction CONFIRMED (24 occurrences). 44.4% plan_json (4/9). 76.1% tool success. 12:02 total. All BPs compile SUCCESS. Single-agent CLI. "Dumb cast" confirmed: RewriteAccessorCalls rewrites GetMesh→GetComponentByClass, downgrades return type to ActorComponent*, forced agent to cast to SceneComponent. Fix: stop rewriting GetMesh or alias to GetSkeletalMeshComponent. DoOnce in FireArrow is logic error (fires once per session). IA_Fire missing → fell back to InputKey. 10 recommendations filed.
- `plans/research/bow-arrow-session-log-analysis-2026-03-09n.md` — Run 09n: Fix verification: PreResolvedFunction CONFIRMED (21 occurrences), bOrphanedPin cleanup NOT FIRING (TypesIncompatible on exec still fires 2×). 36.4% plan_json (4/11) — worst ever. ~85% tool success. 8:14 total — fastest ever. All BPs compile SUCCESS. Single-agent CLI. Agent did 32s of research (3× get_template + 2× get_recipe) before first write. Dominant failures: bOrphanedPin TypesIncompatible (2×, unfixed), @get_class.auto resolver/executor mismatch (1×, new bug), HitResult pin name wrong on ComponentBoundEvent (1×, ImpactResult not HitResult), BreakStruct 0-pin in plan_json (1×, bug), GetMesh type mismatch (1×), missing target_class (1×), LATENT_IN_FUNCTION (1×, correctly caught by Phase 0). GetForwardVector NOT exercised. 10 recommendations filed.
- `plans/research/bow-arrow-session-log-analysis-2026-03-09m.md` — Run 09m: PIPELINE REVERT CONFIRMED. 60.0% plan_json (9/15). 86.4% tool success (70/81). 10:03 total (11.1s discovery + ~9:29 CLI). FASTEST RUN TO DATE. All BPs compile SUCCESS. No multi-agent pipeline activity. Agent used 4× get_template + list_templates before building. Strategic pivot to SetupBow function after bOrphanedPin failures. No node-wipe. Dominant failures: FunctionOutput-as-exec-source (1×, novel), COMPONENT_FUNCTION_ON_ACTOR (1×), data-wire+orphaned-node (1×), bOrphanedPin TypesIncompatible (1×, known bug), GetForwardVector alias-executor mismatch (1×, CONFIRMED BUG), IA_Shoot missing (1×, expected). 2 critical bugs confirmed: GetForwardVector alias fallback lost between resolver and node factory; bOrphanedPin after rollback persists. Full analysis with per-failure root causes filed.
...
- `plans/research/neostack-aura-overview-2026-03-13.md` — Concise overview of NeoStack + Aura: what each is, template approach (neither uses templates), generation approach, key differentiators vs Olive, 6 recommendations. Confirmed 2026-03-13.
...
## Pickup/Equip Pattern Analysis (2026-03-14)
- Full report: `plans/research/pickup-equip-pattern-analysis.md`
- gun.json `usage_notes` has best equip guidance in system (attachment → socket → collision disable) but is only seen when `get_template("gun")` is called
- CRITICAL GAP: `ue_events.json` lists `ActorBeginOverlap` only — does NOT document `OnComponentBeginOverlap`. This causes agents to use the wrong event for pickup spheres.
- Two architecturally distinct pickup patterns exist but are never labeled: (1) AutoPickup (item sphere → overlap → collect) and (2) InteractionPickup (character sphere → press E → BPI)
- fps_shooter library: `BP_InventoryItemBase` uses `OnComponentBeginOverlap` on "Sphere" (correct); `BP_AmmoPickUp_BASE` uses `ReceiveActorBeginOverlap` (wrong but real-world usage)
- Collision disable is context-dependent: disable mesh (StaticMeshComponent) for weapons that stay attached; disable sphere for consumables that disappear/destroy
- combatfs `BP_WeaponParent` uses `K2_AttachToComponent` with socket name from `S_EquipmentInfo` struct — most decoupled design found; has `EquippedMainHandSocket` / `UnEquippedMainHandSocket` fields
- `bIsEquipped` state flag and `EquippedWeapon` character variable are not documented anywhere in templates
- EAttachmentRule values for `K2_AttachToComponent`: `SnapToTarget` (weapons), `KeepRelativeOffset` (IK); only documented in knowledge pack attach example
- 10 recommendations for new `pickup_equip_patterns.json` reference template filed
