...
- `plans/research/bow-arrow-session-log-analysis-2026-03-09t.md` — Run 09t: 7:38 agent time. 77.8% plan_json (7/9). 86.4% tool success (51/59). 59 total tool calls. All 3 BPs compiled SUCCESS. EXEC_SOURCE_IS_RETURN CONFIRMED (1× catch, clean). CleanupStaleEventChains CONFIRMED firing (0 nodes cleaned — no stale event nodes in this run). Exec auto-break CONFIRMED (1×, resolved stale exec wiring). describe_function called 1× (GetMesh on Character — FAILED, C++ accessor not a UFUNCTION). EXEC WIRE REJECTED ZERO (third consecutive clean run). 16 remove_node calls (same as 09s, worst pattern — not from stale events but from partial-commit orphans and connect_pins failures). IA_Fire node created and compiled but then agent deleted it after 1-warning compile, pivoted to InputKey(LeftMouseButton)→Branch→Fire. CustomEvent has no exec INPUT (wired twice by mistake — 09s and 09t, needs knowledge fix). @Mesh ExpandComponentRefs gap: dotless ref passed resolver+Phase0 but failed Phase4 — synthesized 0 steps when it should have made a get_var (only @ArrowSpawnPoint was synthesized correctly). Dominant failures: @Mesh expansion (1×, ExpandComponentRefs context bug), custom_event exec-input wiring mistake (1×, agent knowledge gap). 10 recommendations filed.
