...
## Phase 0: Critical Template & Pipeline Bugs (MUST FIX FIRST)

These are bugs discovered during testing that cause failures BEFORE any self-correction or retry logic fires. They must be fixed before Phases 1-3 can work properly.

### 0A: Template Function Parameter Types Are Wildcard (ROOT CAUSE of BP_Bullet failure)

**Problem**: `ParseTemplateFuncParam()` in OliveTemplateSystem.cpp only reads the simple type string but never reads `class_name`, `struct_name`, `enum_name`, or `is_reference`. For complex types like Object refs and Struct refs, this produces pins with the right category but no concrete type info — which UE treats as Wildcard.

The chain: Template JSON specifies `{"name": "HitActor", "type": "object", "class_name": "Actor"}` → `ParseTemplateFuncParam` reads `type: "object"` → sets `Category = Object` → **ignores `class_name`** → `ClassName` stays empty → `ConvertIRType` creates `PC_Object` with no `PinSubCategoryObject` → UE pin is Wildcard.

This is why both UserDefinedPins AND graph pins show as Wildcard — the type was wrong from the moment the function was created. The UserDefinedPins patching fix was a red herring.

**Downstream consequences**: 
- Auto-match fails (Wildcard ≠ Object Reference)
- Wildcard fallback picks first pin every time → HitActor gets wired to HitInfo slot (should be HitResult)
- Blueprint compile error: "The type of Hit Actor is undetermined"

**Fix — Part 1: `ParseTemplateFuncParam()` in OliveTemplateSystem.cpp**
...
FOliveIRFunctionParam ParseTemplateFuncParam(const TSharedPtr<FJsonObject>& ParamJson)
{
    ...
    // Parse complex type fields needed for Object, Struct, Enum, etc.
    ...
    bool bIsRef = false;
    if (ParamJson->TryGetBoolField(TEXT("is_reference"), bIsRef))
    {
        Param.bIsReference = bIsRef;
    }
...
**Fix — Part 2: Verify/fix the projectile template JSON**

The projectile template's ApplyHitDamage function params must include concrete type info:
```json
{
  "name": "HitActor",
  "type": "object",
...
```json
{
  "name": "HitResult", 
  "type": "struct",
  "struct_name": "FHitResult",
  "is_reference": true
}
```

If the template JSON only has `"type": "object"` without `"class_name"`, add the missing fields. Both parts of the fix are required — parser without template data = still Wildcard, template data without parser = data gets ignored.

**Files**: OliveTemplateSystem.cpp, Content/Templates/factory/projectile.json (or equivalent template file)

### 0B: Skeleton Recompile After add_function (Function-Then-Call Race)

**Problem**: When the AI creates a function via `add_function` (tc_9) and immediately calls it from `apply_plan_json` (tc_11) in the same iteration, `FindFunction('Fire')` fails because the Blueprint's skeleton class hasn't been rebuilt. The node factory searches all library classes and can't find it.

This also affects template execution: the template system creates the function, then immediately runs the function body plan. The function's pins aren't fully resolved.

**Fix**: After creating a function (in both `HandleBlueprintAddFunction` and in the template system's function creation loop), force a skeleton refresh:

```cpp
// After EntryNode->ReconstructNode() and ResultNode->ReconstructNode():
...
This ensures:
1. Function pins get their concrete types resolved (fixes Wildcard even without 0A)
2. The function appears in the generated class (fixes FindFunction race)
3. Subsequent plan_json calls in the same batch can find and wire to the function

**Files**: OliveBlueprintWriter.cpp (AddFunction method), OliveTemplateSystem.cpp (function creation in ApplyTemplate)

### 0C: ResultCards JSON Parse Error (Silent Error Dropping)

**Problem**: Template system builds compile errors as plain strings:
```cpp
CompileErrors.Add(MakeShared<FJsonValueString>(Err.Message));  // String
```
But ResultCards expects objects:
```cpp
if (ErrVal->AsObject().IsValid())  // Calls AsObject() on String → JSON error logged
```
This produces `LogJson: Error: Json Value of type 'String' used as a 'Object'` spam and silently drops compile errors from the UI.

**Fix**: Add string fallback in the ResultCards compile error loop:
```cpp
if (ErrVal->AsObject().IsValid())
{
    Card.Errors.Add(FOliveIRCompileError::FromJson(ErrVal->AsObject()));
}
else if (ErrVal->Type == EJson::String)
{
    FOliveIRCompileError SimpleErr;
    SimpleErr.Message = ErrVal->AsString();
    SimpleErr.Severity = TEXT("Error");
    Card.Errors.Add(SimpleErr);
}
```
Same pattern for the warnings loop below it.

**Files**: OliveResultCards.cpp

### 0D: Wildcard Fallback Disambiguation (Defense in Depth)

**Problem**: Even with the Wildcard fallback in `FindTypeCompatibleOutput`, when there are multiple Wildcard candidates (e.g., HitActor + HitResult both Wildcard), it always picks the first one. This causes wrong wiring (HitActor → HitInfo instead of HitResult → HitInfo).

**Note**: Fixing 0A + 0B eliminates the Wildcard pins, making this moot for the template case. But Wildcards can appear for other reasons (freshly created functions before compile, UE edge cases). This is cheap insurance.

**Fix**: When the Wildcard fallback has multiple candidates, try name-based disambiguation before falling back to first-match:

```cpp
// In FindTypeCompatibleOutput, after Wildcard fallback produces multiple matches:
if (TypeMatches.Num() > 1)
{
    // Try to disambiguate by matching pin name to target pin name
    // (e.g., target input "HitInfo" should prefer source "HitResult" over "HitActor")
    for (const FOlivePinManifestEntry* Pin : TypeMatches)
    {
        if (Pin->PinName == TargetPinName || 
            Pin->DisplayName == TargetPinName)
        {
            return Pin;
        }
    }
    // Fall through to first-match if no name match
}
```

This requires threading the target pin name into `FindTypeCompatibleOutput`. Low priority compared to 0A/0B but prevents silent wrong-wiring.

**Files**: OlivePlanExecutor.cpp

### Phase 0 Verification

1. **Template types** — Run `create_from_template` with projectile/Bullet preset. The `Patched FunctionInput pin` log lines should show concrete types (Object → Object, Struct → Struct) instead of Wildcard → Wildcard. Or better: no patching needed because pins are already correctly typed.
2. **Skeleton recompile** — Create a function via `add_function`, then call it from `apply_plan_json` in the same batch. `FindFunction` should succeed.
3. **ResultCards** — Template compile errors should appear in the chat UI without `LogJson: Error` spam.
4. **End-to-end** — BP_Bullet template should compile successfully on first try with all data wires connected correctly (HitActor → DamagedActor, HitResult → HitInfo, BulletDamage → BaseDamage).
...
**Phase 0 first.** These are critical bugs that cause failures before any retry/self-correction logic fires. Fixing 0A (template param types) + 0B (skeleton recompile) eliminates the BP_Bullet compile failure entirely — no self-correction needed. 0C is a 5-line fix. 0D is optional defense in depth.

**Phase 1 second.** This builds the safety net. With Phase 0 fixing the known root causes, Phase 1 ensures recovery from any FUTURE unknown errors. Without Phase 0, the Phase 1 safety net gets triggered constantly by preventable bugs.

**Phase 2 third.** This threads graph context through the pipeline. It touches resolver, validator, executor, tool handlers, and template system — more files but well-defined changes. Depends on Phase 1 being stable so that any issues during development are caught gracefully instead of death spiraling.

**Phase 3 fourth.** These are isolated lookup table additions. Zero risk, can be done any time. But they're most useful after Phase 2 since the resolver and factory need to agree on what functions exist.
...
| File | Change |
|------|--------|
| `OliveTemplateSystem.cpp` | Fix `ParseTemplateFuncParam()` to read `class_name`, `struct_name`, `enum_name`, `is_reference` |
| `Content/Templates/factory/projectile.json` | Verify/add `class_name` and `struct_name` fields on function params |
| `OliveBlueprintWriter.cpp` | Add skeleton recompile after `AddFunction` (CompileBlueprint with SkipSave) |
| `OliveResultCards.cpp` | Add string fallback when parsing compile error JSON array entries |
| `OlivePlanExecutor.cpp` | *(Optional 0D)* Wildcard fallback name-based disambiguation |
